...
//
// For this exported version, admin authentication is required.
// To integrate a real payment gateway, replace the auth check with
// webhook signature verification.
// ---------------------------------------------------------------------------

export async function POST(request: NextRequest) {
  try {
    // SECURITY: Require admin authentication for payment status changes
    const authHeader = request.headers.get('authorization');
    const admin = await validateSession(authHeader);
    if (!admin) {
    ...
    // Idempotency: if already SUCCESS, return safe fields only
